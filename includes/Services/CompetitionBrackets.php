...
class CompetitionBrackets {
    ...
    public function render_meta_box( WP_Post $post ): void {
        ...
        if ( $snapshot ) {
            ...
            echo '<p>اندازه: ' . (int) $snapshot['size'] . ' نفره | معیار جداسازی: ' . esc_html( self::criterion_label( (string) $snapshot['criterion'] ) ) . ' | تولید: ' . esc_html( (string) $snapshot['generated_at'] ) . '</p>';
    ...
    private function render_result_fields( array $snapshot ): void {
        $rounds = self::build_rounds( $snapshot['slots'], $snapshot['winners'] ?? [] );
        ...
        foreach ( $rounds as $round_index => $matches ) {
            foreach ( $matches as $match_index => $match ) {
                ...
                $a = $match['a'];
                $b = $match['b'];
                echo '<tr><td>' . esc_html( self::round_label( count( $snapshot['slots'] ), $round_index ) ) . '</td><td>' . ( $match_index + 1 ) . '</td><td>' . esc_html( self::entry_name( $a ) ) . '</td><td>' . esc_html( self::entry_name( $b ) ) . '</td><td>';
                if ( $a && $b ) {
                    echo '<select name="imao_bracket_winners[' . esc_attr( $key ) . ']"><option value="">— انتخاب —</option>';
    ...
    public function save_admin_changes( int $competition_id, WP_Post $post ): void {
        ...
        if ( $action === 'generate' ) {
            ...
            if ( count( $participants ) < 2 || count( $participants ) > self::MAX_SIZE ) return;
            $criterion = $this->criterion( $competition_id );
            $seed = bin2hex( random_bytes( 16 ) );
            $slots = self::seed_participants( $participants, self::bracket_size( count( $participants ) ), $criterion, $seed );
            update_post_meta( $competition_id, self::META_BRACKET, [
                ...
                'audit' => [ [ 'at' => current_time( 'mysql' ), 'user_id' => get_current_user_id(), 'message' => 'جدول با ' . count( $participants ) . ' شرکت‌کننده تولید شد.' ] ],
            ] );
    ...
    private function bracket_html( int $competition_id ): string {
        ...
        $rounds = self::build_rounds( $snapshot['slots'], $snapshot['winners'] ?? [] );
        ob_start(); ?>
<style>.imao-bracket-shell{direction:rtl;background:#fff;border:1px solid #e2e5ea;border-radius:14px;overflow:hidden;font-family:Tahoma,Arial,sans-serif}.imao-bracket-toolbar{display:flex;gap:8px;align-items:center;padding:10px 14px;background:#17134b;color:#fff}.imao-bracket-toolbar button{border:1px solid #ffffff66;border-radius:6px;background:#fff;color:#17134b;padding:5px 11px;cursor:pointer}.imao-bracket-viewport{direction:ltr;overflow:auto;cursor:grab;min-height:420px;padding:24px}.imao-bracket-tree{display:flex;gap:54px;transform-origin:top left;width:max-content;transition:transform .15s}.imao-bracket-round{display:flex;flex-direction:column;justify-content:space-around;width:220px;gap:18px}.imao-bracket-round h3{direction:rtl;text-align:center;font-size:14px;margin:0 0 8px;color:#555}.imao-bracket-match{position:relative;direction:rtl;border:1px solid #ccd2dd;border-radius:8px;background:#fff;box-shadow:0 2px 8px #0000000d;overflow:visible}.imao-bracket-match:after{content:"";position:absolute;left:100%;top:50%;width:55px;border-top:2px solid #aab2c0}.imao-bracket-round:last-child .imao-bracket-match:after{display:none}.imao-bracket-player{display:flex;justify-content:space-between;gap:8px;padding:8px 10px;min-height:35px;border-bottom:1px solid #edf0f4;font-size:12px}.imao-bracket-player:last-child{border:0}.imao-bracket-player.winner{background:#eaf8ee;color:#116b2d;font-weight:700}.imao-bracket-player.bye{color:#9299a5;font-style:italic;direction:ltr}.imao-bracket-champion{border-color:#d7a900;box-shadow:0 0 0 2px #ffd84d55}.imao-bracket-button{display:inline-block;padding:10px 18px;border-radius:7px;background:#17134b;color:#fff!important;text-decoration:none}.imao-bracket-empty{text-align:center;color:#777}</style>
<div class="imao-bracket-shell" data-imao-bracket><div class="imao-bracket-toolbar"><strong>جدول <?php echo (int) $snapshot['size']; ?> نفره</strong><button type="button" data-zoom="in">+</button><button type="button" data-zoom="out">−</button><button type="button" data-zoom="reset">۱۰۰٪</button></div><div class="imao-bracket-viewport"><div class="imao-bracket-tree">
<?php foreach ( $rounds as $round_index => $matches ) : ?><section class="imao-bracket-round"><h3><?php echo esc_html( self::round_label( (int) $snapshot['size'], $round_index ) ); ?></h3><?php foreach ( $matches as $match ) : ?><div class="imao-bracket-match <?php echo $round_index === count( $rounds ) - 1 && $match['winner'] ? 'imao-bracket-champion' : ''; ?>"><?php foreach ( [ $match['a'], $match['b'] ] as $entry ) : ?><div class="imao-bracket-player <?php echo ! $entry ? 'bye' : ( $match['winner'] && $entry['entry_id'] === $match['winner']['entry_id'] ? 'winner' : '' ); ?>"><span><?php echo esc_html( self::entry_name( $entry ) ); ?></span><?php if ( $entry && ! empty( $entry['group_label'] ) ) : ?><small><?php echo esc_html( $entry['group_label'] ); ?></small><?php endif; ?></div><?php endforeach; ?></div><?php endforeach; ?></section><?php endforeach; ?>
</div></div></div><script>(function(){document.querySelectorAll('[data-imao-bracket]').forEach(function(root){if(root.dataset.ready)return;root.dataset.ready='1';var view=root.querySelector('.imao-bracket-viewport'),tree=root.querySelector('.imao-bracket-tree'),scale=1,drag=false,x=0,y=0,sx=0,sy=0;function draw(){tree.style.transform='scale('+scale+')'}root.addEventListener('click',function(e){var z=e.target.dataset.zoom;if(!z)return;scale=z==='reset'?1:Math.max(.4,Math.min(2,scale+(z==='in'?.15:-.15)));draw()});view.addEventListener('mousedown',function(e){drag=true;x=e.clientX;y=e.clientY;sx=view.scrollLeft;sy=view.scrollTop});window.addEventListener('mouseup',function(){drag=false});view.addEventListener('mousemove',function(e){if(!drag)return;view.scrollLeft=sx-(e.clientX-x);view.scrollTop=sy-(e.clientY-y)});view.addEventListener('wheel',function(e){if(!e.ctrlKey)return;e.preventDefault();scale=Math.max(.4,Math.min(2,scale+(e.deltaY<0?.1:-.1)));draw()},{passive:false})})})();</script>
<?php return (string) ob_get_clean();
    }

    private static function snapshot( int $competition_id ): array {
        $value = get_post_meta( $competition_id, self::META_BRACKET, true );
        return is_array( $value ) && ! empty( $value['slots'] ) ? $value : [];
    }
    ...
    private function participants( int $competition_id ): array {
        ...
        if ( $product_id && function_exists( 'wc_get_orders' ) ) {
            ...
            foreach ( $order_ids ? wc_get_orders( [ 'limit' => -1, 'status' => [ 'processing', 'completed' ], 'include' => array_map( 'intval', $order_ids ) ] ) : [] as $order ) {
                foreach ( $order->get_items() as $item ) {
                    ...
                    $entry = $this->participant( (int) $order->get_customer_id(), 'order-' . $order->get_id() . '-item-' . $item->get_id() );
        ...
        foreach ( array_map( 'intval', (array) get_post_meta( $competition_id, self::META_MANUAL, true ) ) as $user_id ) {
            ...
            $entry = $this->participant( $user_id, 'manual-' . $user_id );
            if ( $entry ) $entries[] = $entry;
        }
        return $entries;
    }

    private function participant( int $user_id, string $entry_id ): ?array {
