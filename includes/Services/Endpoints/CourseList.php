...
class CourseList
{
    ...
    private function render(): string
    {
        ...
        $q = new WP_Query(['post_type' => 'course', 'posts_per_page' => -1, 'orderby' => 'date', 'order' => 'DESC', 'tax_query' => [['taxonomy' => 'gender', 'field' => 'slug', 'terms' => $gender,], ['taxonomy' => 'age_category', 'field' => 'slug', 'terms' => $age_slug,],],]);
        $posts = [];
        while ($q->have_posts()) {
            $q->the_post();
            $cid = get_the_ID();
        ...
        <div class="crm-course-wrap">
            ...
            <table class="crm-course-table striped">
                ...
                <tbody>
                <?php $i = 1;
                foreach ($posts as $post) : $cid = $post->ID; ?>
                    <tr>
                        <td><?php echo $i++; ?></td>
                        ...
                        <?php endforeach; ?>
                        <td><a href="<?php echo esc_url('/my-account/course-details/?course_id=' . $cid); ?>">جزئیات /
                                ثبت‌نام</a></td>
                    </tr>
                <?php endforeach; ?>
                </tbody>
