...
class CompetitionCards {
    ...
    private function card_data( WC_Order $order, WC_Order_Item_Product $item, int $competition_id ): array {
        ...
        return [
            ...
            'verification' => strtoupper( substr( hash_hmac( 'sha256', $order->get_id() . '|' . $item->get_id() . '|' . $user_id, wp_salt( 'auth' ) ), 0, 16 ) ),
        ];
    }
...
<div class="verification"><small>کد اصالت کارت</small><?php echo esc_html( $data['verification'] ); ?></div>
